      ...
      <ha-card>
        ...
        <div class="timer-container">
          <svg class="timer-svg" viewBox="0 0 400 400">
            ...
            <!-- Dividing lines -->
            ${Array.from({length:24},(t,e)=>{const i=(360*e/24-90)*(Math.PI/180),a=s+r*Math.cos(i),c=n+r*Math.sin(i),l=s+o*Math.cos(i),h=n+o*Math.sin(i);return W`
                <line 
                  x1="${a}" 
                  y1="${c}" 
                  x2="${l}" 
            ...
            ${Array.from({length:24},(t,e)=>{const r=e,c=i.find(t=>t.hour===r&&0===t.minute),l=c?.isActive||!1,h=this.currentTime.getHours()===r&&this.currentTime.getMinutes()<30,d=this.createSectorPath(r,24,a,o,s,n),p=this.getTextPosition(r,24,147.5,s,n),u=this.getSectorCenterAngleDeg(r,24),f=this.getUprightTextRotationDeg(u),g=p.y+3;return W`
                <path 
                  d="${d}" 
                  fill="${l?"#10b981":"#ffffff"}"
                  stroke="${h?"#ff6b6b":"#e5e7eb"}"
                  stroke-width="${h?"3":"1"}"
                  style="cursor: pointer; transition: all 0.2s;"
                  @click="${t=>{this.handleSlotClick(t,r,0)}}">
                </path>
                <text 
                  x="${p.x}" 
                  y="${g}" 
            ...
            ${Array.from({length:24},(t,e)=>{const o=e,c=i.find(t=>t.hour===o&&30===t.minute),l=c?.isActive||!1,h=this.currentTime.getHours()===o&&this.currentTime.getMinutes()>=30,d=this.createSectorPath(o,24,r,a,s,n),p=this.getTextPosition(o,24,82.5,s,n),u=this.getSectorCenterAngleDeg(o,24),f=this.getUprightTextRotationDeg(u),g=p.y+2;return W`
                <path 
                  d="${d}" 
                  fill="${l?"#10b981":"#f8f9fa"}"
                  stroke="${h?"#ff6b6b":"#e5e7eb"}"
                  stroke-width="${h?"3":"1"}"
                  style="cursor: pointer; transition: all 0.2s;"
                  @click="${t=>{this.handleSlotClick(t,o,30)}}">
                </path>
                <text 
                  x="${p.x}" 
                  y="${g}" 
